...
class FunSetSuite extends munit.FunSuite:
  ...
  test("intersect contains all elements all elements that are both in `s` and `t`") {
    new TestSets:
  ...
  test("intersect contains all elements all elements that are both in `s` and `t`") {
    new TestSets:
      ...
      assert(contains(s, 2), "Intersec5 2")
